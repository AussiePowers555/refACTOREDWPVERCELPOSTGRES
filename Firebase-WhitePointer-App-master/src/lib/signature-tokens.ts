...
/**
 * Update form data for a signature token (for draft saving)
 */
export async function updateSignatureTokenFormData(
  token: string,
  formData: any
): Promise<void> {
  try {
    ...
    console.log('✅ Signature token form data updated:', token);
  } catch (error) {
    console.error('Error updating signature token form data:', error);
    throw new Error('Failed to update signature token form data');
  }
}
